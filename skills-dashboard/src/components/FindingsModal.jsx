...
function SevBadge({ sev }) {
  ...
  return (
    <span style={{ color: c, borderColor: c }}
    ...
    >
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3 p-4 border-b border-dashed border-[#393939]">
          <div>
            <div className="text-[10px] uppercase tracking-wide text-[#6b6b6b] mb-0.5">Security Findings</div>
            <div className="font-bold text-sm font-mono text-[#3b6fd4] break-all">{skillPath}</div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-wrap gap-3 px-4 py-2.5 border-b border-dashed border-[#e4e0d6]">
          <span className="text-[11px] text-[#6b6b6b]">{findings.length} finding{findings.length !== 1 ? 's' : ''} total</span>
          {['CRITICAL','HIGH','MEDIUM','LOW'].filter(s => counts[s]).map(s => (
            <span key={s} className="text-[11px]" style={{ color: SEV_COLORS[s] }}>
              {counts[s]} {s}
            </span>
          ))}
        </div>
          ...
          {findings.map((f, i) => (
            <div key={i} className="px-4 py-3">
              <div className="flex items-start gap-2 mb-1">
                ...
                <span className="text-[11px] font-semibold text-[#141414] leading-tight">{f.title}</span>
                <span className="ml-auto text-[10px] text-[#9e9e9e] shrink-0">
                  {RULE_LABELS[f.rule_id] || f.rule_id}
                </span>
              </div>
              {f.description && (
                <p className="text-[11px] text-[#474747] mb-1 ml-0.5">{f.description}</p>
              )}
              {f.evidence && (
                <div className="ml-0.5 mt-1">
                  <span className="text-[10px] uppercase text-[#9e9e9e] tracking-wide">Evidence</span>
                  <pre className="mt-0.5 text-[10px] bg-[#eee9d7] text-[#393939] p-1.5 overflow-x-auto whitespace-pre-wrap break-all leading-snug">
                    {f.line > 0 && <span className="text-[#9e9e9e]">L{f.line}: </span>}{f.evidence}
                  </pre>
                </div>
              )}
              {f.recommendation && (
                <p className="text-[10px] text-[#2f7d31] mt-1 ml-0.5 italic">↳ {f.recommendation}</p>
              )}
            </div>
          ))}
        </div>
