  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4">
      <div className="border border-dashed border-[#c6392c] p-8 max-w-md w-full">
        <div className="text-sm font-bold text-[#c6392c] mb-2">Failed to load catalog</div>
        <div className="text-[12px] text-[#474747] font-mono whitespace-pre-wrap">{message}</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="min-h-screen bg-[#fbf7eb]">
      ...
      <footer className="border-t border-dashed border-[#393939] mt-8 py-3 text-center text-[10px] text-[#9e9e9e]">
        OpenClaw Skills Hub · {data.stats.total_active?.toLocaleString()} skills · DuckDB WASM
      </footer>
    </div>
  )
}
